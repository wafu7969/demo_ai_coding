...
def classify_request(state: State) -> State:
    ...
    sys = """
    只输出：dev 或 bugfix 或 qna，不要任何其他字符。
    判定规则：
    - dev：包含新增/实现/搭建/开发/生成/编写/创建/设计/集成/改造等意图或要求生成文件/接口/组件/SQL/项目结构。
    - bugfix：出现报错/错误/异常/崩溃/无法运行/堆栈/错误码/日志并意图修复或定位问题”。
    - qna：知识性或科普问答（如 原理/是什么/怎么做/区别/示例/最佳实践），不涉及落地实现或修复。
    优先级：若同时有错误日志并要求修复，选择 bugfix；若既有知识问答也包含明确开发需求，选择 dev。
    示例：
    - “开发一个OA系统，提供登录与审批” → dev
